...
extern "system" fn MessageCallback(
    _source: u32,
    type_: u32,
    _id: u32,
    severity: u32,
    ...
    unsafe {
        ...
        eprintln!(
            ...
            if type_ == gl::DEBUG_TYPE_ERROR { "** GL ERROR **" } else { "" },
            type_, severity, message.to_str().unwrap(),
        );
    }
}
...
pub fn load(gl_context: &glutin::Context<PossiblyCurrent>) -> Gl {
    ...
    /*unsafe {
        ...
        gl.DebugMessageCallback(Some(MessageCallback), std::ptr::null_mut());
    }*/
...
impl Gl {
    pub fn draw_frame(&self, _color: [f32; 4]) {
        /*unsafe {
